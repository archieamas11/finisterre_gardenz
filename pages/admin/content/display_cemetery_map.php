...
<!-- Include Cemetery Popup Templates -->
...
<script>
    // Make userType globally available for imported scripts
    ...
    // Make autolinker globally available for imported scripts
    ...
    // ============================================
    // Template functions have been moved to: assets/js/map/cemetery-popup-templates.js
    // Functions available: getButtons, empty, getReservedTemplate, getMultipleGraveTemplate, 
    // getSingleGraveTemplate, getLocationSection, getTimelineSection, getPhotosSection, getPopupTemplate
        ...
        return `
            <div class="buttons">
                ...
                <span class="edit-record-btn">
                    <a href="index.php?id=${feature.properties['id'] !== null ? feature.properties['id'] : ''}&grave_no=${feature.properties['Grave No.'] ?? ''}&page=edit_record">
    ...
    function pop_pointsfinall_3(feature, layer) {
        // Import template function to get popup content
        const popupContent = getPopupTemplate(feature);
        function getPopupTemplate(feature) {
            ...
            if (visibility === 'private' && userType === 'user') {
                return empty(feature);
            }
            
            // Display reserved template if grave status is 'reserved'
            if (graveStatus === 'reserved') {
                return getReservedTemplate(feature);
            }
            
            // Display empty template if the deceased count is less than 1 or the grave status is vacant
            if (deceasedCount < 1 || graveStatus === 'vacant') {
                return empty(feature);
            }
            
            // Display multiple grave template if the deceased count is greater than 1
            if (deceasedCount > 1) {
                return getMultipleGraveTemplate(feature);
            }
            
            // Default to single grave template
            return getSingleGraveTemplate(feature);
        }
    ...
    var refreshControl = L.Control.extend({
        ...
        onAdd: function(map) {
            ...
            var button = L.DomUtil.create('a', 'refresh-button', container);
            ...
            L.DomEvent.on(button, 'click', function(e) {
                ...
                map.setMinZoom(defaultView.minZoom);
                map.setMaxZoom(defaultView.maxZoom);
            });
