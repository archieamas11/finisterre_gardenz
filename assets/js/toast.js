const toastContainer = document.getElementById("sonner-toast-container");
...
function createToastElement(options) {
  ...
  toast.className = `sonner-toast ${options.type ? `sonner-toast-${options.type}` : ""}`;
  toast.dataset.toastId = options.id;

  // Main content wrapper (new)
  const mainContent = document.createElement("div");
  mainContent.className = "sonner-toast-main-content";

  // Header (new - for icon, title, and timestamp)
  const headerElement = document.createElement("div");
  headerElement.className = "sonner-toast-header";

  const titleIconWrapper = document.createElement("div"); // New wrapper for icon and title
  titleIconWrapper.className = "sonner-toast-title-icon-wrapper";

  // Icon (modified)
  if (options.icon !== false) {
    const iconElement = document.createElement("div");
    iconElement.className = "sonner-toast-icon";
    // Using a simple checkmark SVG for success, as in the image
    if (options.type === "success" || options.type === "default") {
      // Default to success icon for the example
  ...
  if (options.title) {
    ...
    titleElement.className = "sonner-toast-title";
  ...
  timestampElement.className = "sonner-toast-timestamp";
  ...
  if (options.description) {
    ...
    descriptionElement.className = "sonner-toast-description";
  ...
  if (options.closable !== false) {
    ...
    closeButton.className = "sonner-toast-close-button";
...
function showToast(options = {}) {
  ...
  const mergedOptions = {
    ...
    description:
      "A new service request (ID: SR20240721001) for plot maintenance has been created.", // Default description from image
...
function updateToastPositions() {
  const activeToasts = toasts.filter(
    (t) => !t.element.classList.contains("exiting"),
  );
  ...
  if (toastContainer.classList.contains("expanded-stack")) {
    ...
    const visibleToastsInStack = activeToasts
    ...
    for (let i = 0; i < visibleToastsInStack.length; i++) {
      ...
      const opacity =
        i === 0 ? 1 : Math.max(0.1, 1 - i * STACK_ITEM_OPACITY_DECREMENT);
  ...
  toastContainer.style.pointerEvents =
    activeToasts.length > 0 ? "auto" : "none";
}
...
toastContainer.addEventListener("mouseenter", () => {
  ...
  hoverDebounceTimer = setTimeout(() => {
    ...
    if (
      toasts.filter((t) => !t.element.classList.contains("exiting")).length > 0
    ) {
      if (!toastContainer.classList.contains("expanded-stack")) {
